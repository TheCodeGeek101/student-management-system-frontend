...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const {  data, adminId }: RequestBody = req.body;
  if (!adminId || !data) {
    return res
      .status(400)
  ...
  const backendURL = `${process.env.NEXT_PUBLIC_API_URL}/api/payments/admin/${adminId}`;
  try {
    const response = await axios.post(backendURL, data);
    console.log(`Data fetched for payments:`, response.data);
