...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { updateEndPoint, formData,  assessmentId } = req.body;

  if (!updateEndPoint || !formData ||  !assessmentId) {
    return res.status(400).json({ message: 'Missing required parameters.' });
  }

  const backendURL = `${process.env.NEXT_PUBLIC_API_URL}/api`;
  const url = `${backendURL}/${encodeURIComponent(updateEndPoint)}/assessment/${encodeURIComponent(assessmentId)}/update`;
